...
struct Emitter {
  ...
  void run() {
    ...
    for (const IrFunc &f : mod.funcs)
      for (Type *t : f.value_type)
        if (t->kind == TY_SLICE || t->kind == TY_STRING ||
            (t->kind == TY_PTR && (t->elem->kind == TY_SLICE ||
                                   t->elem->kind == TY_STRING)))
          needs_slice = true;
    if (!mod.strings.empty()) needs_slice = true;
    if (needs_slice) fputs("%slice = type { ptr, i64 }\n", out);
    ...
    if (needs_slice || !mod.structs.empty()) fputc('\n', out);
